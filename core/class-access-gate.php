...
final class Access_Gate {
	...
	public static function maybe_handle_unlock_post(): bool {
		...
		$pass = isset( $_POST['nexulesuite_gate_password'] ) ? sanitize_text_field( wp_unslash( (string) $_POST['nexulesuite_gate_password'] ) ) : '';
		if ( '' === $pass ) {
			return false;
		}
	...
	public static function render_inline_lock_form_html( string $title, string $message = '' ): string {
		...
		$html .= '<input id="' . esc_attr( $id_input ) . '" data-nexulesuite_gate-pass="1" type="password" name="nexulesuite_gate_password" autocomplete="current-password" inputmode="numeric" style="width:100%;height:44px;padding:0 44px 0 14px;border:1px solid #e2e8f0;border-radius:16px;background:#f8fafc;color:#0f172a;font-size:14px;outline:none;box-sizing:border-box;" />';
